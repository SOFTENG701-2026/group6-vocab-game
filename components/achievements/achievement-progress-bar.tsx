...
export default function AchievementProgressBar({ current, required }: AchievementProgressBarProps) {
  ...
  return (
    <div className='relative h-9 w-full overflow-hidden rounded-full bg-gray-200'>
      <div
        className='h-full rounded-full bg-green-400 transition-all duration-300'
        style={{ width: `${progressPercent}%` }}
      />

      <p className='absolute inset-0 flex items-center justify-center text-lg font-black text-black'>
        {current} / {required}
      </p>
    </div>
  );
